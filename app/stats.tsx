  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0a0a0a' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 60, paddingHorizontal: 24, paddingBottom: 20 }}>
        ...
        <Text style={{ color: '#ffffff', fontSize: 24, fontWeight: '700' }}>Stats</Text>
      </View>
      ...
      {isEmpty ? (
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40 }}>
          ...
          <Text style={{ color: '#666666', fontSize: 13, fontWeight: '500', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12, marginTop: 8 }}>
            Top Cravings
          </Text>
          {topCravings.map((item) => {
            ...
            return (
              <View key={item.craving_id} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                ...
                <Text style={{ color: '#cccccc', fontSize: 13, width: 120 }} numberOfLines={1}>
                  {item.craving_label}
                </Text>
                <View style={{ flex: 1, backgroundColor: '#1a1a1a', borderRadius: 4, height: 14, marginHorizontal: 8, overflow: 'hidden' }}>
                  <View style={{ height: 14, borderRadius: 4, backgroundColor: '#6366f1', width: barWidth }} />
                </View>
                <Text style={{ color: '#666666', fontSize: 12, width: 24, textAlign: 'right' }}>{item.count}</Text>
              </View>
            );
          })}
          ...
          {filteredFeedback.length > 0 && (
            <>
              <Text style={{ color: '#666666', fontSize: 13, fontWeight: '500', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12, marginTop: 28 }}>
                What Helped
              </Text>
              {filteredFeedback.map((stat) => {
                ...
                const barColor = yesRate >= 50 ? '#22c55e' : '#ff6b6b';
                return (
                  <View key={stat.craving_id} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <View style={{ flex: 1, backgroundColor: '#1a1a1a', borderRadius: 4, height: 10, marginHorizontal: 8, overflow: 'hidden' }}>
                      <View style={{ height: 10, borderRadius: 4, backgroundColor: barColor, width: `${yesRate}%` as DimensionValue }} />
                    </View>
                    <Text style={{ color: '#666666', fontSize: 12, width: 36, textAlign: 'right' }}>{yesRate}%</Text>
                  </View>
                );
              })}
            </>
